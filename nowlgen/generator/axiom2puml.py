...
from .utils.strings import python_name_to_xsd, clean_puml_output, check_duplicate_lines
...
def get_axiom(class_entity, ontology, type):
    #equivalent to
    if type == "ns":
        return (class_entity.equivalent_to or class_entity.INDIRECT_equivalent_to or [None])[0]
    # general_class_axiom
    elif type == "s":
...
class AxiomToPumlConverter:
    ...
    def __init__(self, ontology, class_entities, types=None, layout_type=None, layout_params=None, 
                 visualize=False, save_puml = True, check_duplicates=True, nowl_profile_path = './nowl/nowl.iuml'):
        # Handle multiple ontology files
        self.ontologies = []
        if isinstance(ontology, (list, tuple)):
            # Multiple ontology files
            for i, onto_file in enumerate(ontology):
                try:
                    loaded_onto = get_ontology(onto_file).load() if isinstance(onto_file, str) else onto_file
                    self.ontologies.append(loaded_onto)
                    if isinstance(onto_file, str):
                        check_ontology_imports(loaded_onto)
                except Exception as e:
                    print(f"Warning: Could not load ontology {onto_file}: {e}")
                    continue
            
            if not self.ontologies:
                raise ValueError("No ontologies could be loaded successfully")
            self.ontology = self.ontologies[0]
            print(f"Successfully loaded {len(self.ontologies)} ontology file(s)")
    ...
    def get_class_name(self, entity):
        ...
        if isinstance(entity, ThingClass):
            ...
            if class_name not in self.class_map:
                ...
                self.puml_output.append(f'class({node_id}, {prefix}{label})')
    ...
    def convert(self):
        ...
        if self.check_duplicates:
            self.puml_output = clean_puml_output(self.puml_output, report=True)
        # Apply layout
        if self.layout_type is not None and self.graph.number_of_nodes() > 0:
            uniform_direction = ["up", "u", "down", "d", "left", "l", "right", "r"]
            if self.layout_type in uniform_direction:
                edge_directions = {(src, tgt): self.layout_type for src, rel, tgt in self.relationships}
        ...
        if self.save_puml:
            class_names = []
            for entity in self.class_entities:
                if hasattr(entity, 'name'):
                    class_names.append(entity.name)
                else:
                    class_names.append(str(entity).split('.')[-1])
            if len(class_names) == 1:
                base_name = class_names[0]
            elif len(class_names) <= 3:
                base_name = "_".join(class_names)
            else:
                base_name = "_".join(class_names[:3]) + f"_and_{len(class_names)-3}_more"
            unique_types = list(set(self.types))
            types_str = "_".join(sorted(unique_types))
            ...
            with open(file_path, "w") as f:
                ...
                print(f"PUML file saved as {file_path}")
        else:
            file_path = None
        
        return "\n".join(self.puml_output), file_path