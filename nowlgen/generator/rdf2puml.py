...
from .utils.strings import clean_puml_output, check_duplicate_lines
from .utils.label import get_label, get_prefix
...
class RdfToPumlConverter:
    def __init__(self, input, import_ontologies=[], save_puml=True, layout_type=None, layout_params=None, 
                 visualize=False, save_viz=None, figsize=(10, 8), exclude_relation=None, inline_class_declaration=False, 
                 check_duplicates=True, nowl_profile_path='./nowl/nowl.iuml',):
        self.classes = {}
        self.individuals = {}
        self.properties = []
        self.input = input
        ...
        self.exclude_relation = exclude_relation or []
        self.inline_class_declaration = inline_class_declaration
        self.check_duplicates = check_duplicates
        self.nowl_profile_path = nowl_profile_path
    ...
    def load_data(self):
        ...
        try:
            ...
            raise Exception(f"Failed to extract individuals from ontology '{self.input}'")

        
        # # Debug: Print individual details
        # for ind in all_individuals:
        #     print(f"Individual: {ind.name}")
        #     print(f"  Classes (is_a): {[str(c) for c in ind.is_a]}")
        #     print(f"  Properties: {[str(p) for p in ind.get_properties()]}")
        #     for prop in ind.get_properties():
        #         print(f"    {prop}: {prop[ind]}")
        ...
        for ind in all_individuals:
            ...
            if ind.is_a:
                for class_object in ind.is_a:
                    try:
                        if hasattr(class_object, 'name') and class_object.name == 'Thing':
                            continue
                        class_label = get_label(class_object)
                        self.classes[class_label] = class_object
                        if "typeOf" not in self.exclude_relation:
                            self.properties.append((ind, "typeOf", class_label))
                            # print(f"Added typeOf: {ind.name} -> {class_label}")
                    except Exception as e:
                        print(f"Error processing class for {ind.name}: {e}")
                        continue 
            #get ind prop
            for prop in ind.get_properties():
                # Skip this property if it's in the excluded relations
                if prop.iri in self.exclude_relation:
                    print(f"Skipping excluded property: {prop.iri}")
                    continue
                
                for value in prop[ind]:
                    if hasattr(prop, 'inverse') and prop.inverse:
                        # Skip if the inverse property is in the excluded relations
                        if prop.inverse.iri in self.exclude_relation:
                            # print(f"Skipping excluded inverse property: {prop.inverse.iri}")
                            continue
                        # Avoid duplicate property entries
                        if (value, prop.inverse, ind) in self.properties:
                            # print(f"Skipping duplicate inverse property: {value} -{prop.inverse}-> {ind}")
                            continue
                    
                    if (ind, prop, value) not in self.properties:
                        self.properties.append((ind, prop, value))
                        # print(f"Added property: {ind.name} -{prop}-> {value}")
    ...
    def filter_isolated_nodes(self):
        # print(f"\nBefore filtering isolated nodes:")
        # print(f"  Classes: {len(self.classes)}")
        # print(f"  Individuals: {len(self.individuals)}")
        connected_nodes = set()
        for s, p, o in self.properties:
            if p == "typeOf":
                if hasattr(s, 'name'):
                    connected_nodes.add(s.name)
                    # print(f"Connected (typeOf subject): {s.name}")
                connected_nodes.add(o)
                # print(f"Connected (typeOf object): {o}")
            elif isinstance(s, object) and hasattr(s, 'name') and isinstance(o, object) and hasattr(o, 'name'):
                connected_nodes.add(s.name)
                connected_nodes.add(o.name)
                # print(f"Connected (object-object): {s.name}, {o.name}")
            elif isinstance(s, object) and hasattr(s, 'name'):
                connected_nodes.add(s.name)
                # print(f"Connected (subject): {s.name}")
                if isinstance(o, str):
                    connected_nodes.add(o)
        #             print(f"Connected (string object): {o}")
        
        # print(f"Connected nodes: {connected_nodes}")
        
        # # Filter individuals and classes to keep only connected ones
        # original_individuals = len(self.individuals)
        # original_classes = len(self.classes)
        ...
        # print(f"\nAfter filtering isolated nodes:")
        # print(f"  Classes: {len(self.classes)} (removed {original_classes - len(self.classes)})")
        # print(f"  Individuals: {len(self.individuals)} (removed {original_individuals - len(self.individuals)})")
        
        if len(self.individuals) == 0 and len(self.classes) == 0:
            print("WARNING: All nodes were filtered out! This suggests a problem with relationship processing.")
    ...
    # Inline class declaration mode
        ...
        if self.check_duplicates:
            puml_lines = clean_puml_output(puml_lines, report=True)    
        ...
        if self.save_puml:
            if self.input.startswith("http://") or self.input.startswith("https://"):
                base_name = os.path.basename(self.input)
                if not base_name:
                    base_name = os.path.basename(os.path.dirname(self.input))
